...
2. Create or update an OpenSpec change for non-trivial work. OpenSpec change
   state is tracked in GitHub issues (one issue per change, labelled `openspec`)
   through the repository adapter `tools/openspec-issue`, not in per-change
