...
class IGCParserService {
  ...
  FlightPoint? _parseBRecord(
    ...
    // GGGGG (31..35)
    try {
      final hh = int.parse(line.substring(1, 3));
      final mm = int.parse(line.substring(3, 5));
      final ss = int.parse(line.substring(5, 7));
      ...
      final latDeg = double.parse(line.substring(7, 9));
      final latMin = double.parse(line.substring(9, 14)) / 1000.0;
      final latHemi = line.substring(14, 15);
      var latitude = latDeg + (latMin / 60.0);
      if (latHemi == 'S') latitude = -latitude;

      final lonDeg = double.parse(line.substring(15, 18));
      final lonMin = double.parse(line.substring(18, 23)) / 1000.0;
      final lonHemi = line.substring(23, 24);
      var longitude = lonDeg + (lonMin / 60.0);
      if (lonHemi == 'W') longitude = -longitude;

      final pressAlt = double.parse(line.substring(25, 30));
      final gnssAlt = double.parse(line.substring(30, 35));
      ...
      // If vario or speed not provided in extensions, calculate from previous point
