...
void main() {
  group('LocalTileServer', () {
    ...
    setUp(() {
      ...
      overviewPath =
          File('assets/map_data/global_overview.pmtiles').existsSync()
    ...
    test('serves local PMTiles tile with gzip and protobuf headers', () async {
      ...
      final req = await client.getUrl(
        Uri.parse('${server.baseUrl}/tiles/0/0/0.pbf'),
      );
      final resp = await req.close();
      expect(resp.statusCode, equals(HttpStatus.ok));
      expect(
    ...
    test(
      'serves /tiles.json with correct TileJSON schema and tiles URL',
      () async {
        await server.start();
        await server.setFallbackArchive(overviewPath);

        final req = await client.getUrl(
          Uri.parse('${server.baseUrl}/tiles.json'),
        );
        final resp = await req.close();
        expect(resp.statusCode, equals(HttpStatus.ok));
        ...
        expect(
          json['tiles'],
          equals(['${server.baseUrl}/tiles/{z}/{x}/{y}.pbf']),
        );
    ...
    test(
      'returns 204 No Content for missing tile when online fallback is disabled',
      () async {
        await server.start();
        await server.setFallbackArchive(overviewPath);

        final req = await client.getUrl(
          Uri.parse('${server.baseUrl}/tiles/1/0/0.pbf'),
        );
        final resp = await req.close();
    ...
    test('returns 404 for unknown route', () async {
      ...
      final req = await client.getUrl(
        Uri.parse('${server.baseUrl}/unknown-path'),
      );
      final resp = await req.close();
    ...
    test(
      'retains resilient default snapshot template and supports dynamic discovery',
      () async {
    ...
    test(
      'proxies online tile, caches to disk, and serves offline on subsequent requests',
      () async {
        final mockUpstream = await HttpServer.bind(
        ...
        final fallbackServer = LocalTileServer(
          ...
          onlineFallbackUrlTemplate:
              'http://127.0.0.1:${mockUpstream.port}/tiles/{z}/{x}/{y}.pbf',
        ...
        try {
          ...
          final req1 = await client.getUrl(
            Uri.parse('${fallbackServer.baseUrl}/tiles/5/10/15.pbf'),
          );
          final resp1 = await req1.close();
          expect(resp1.statusCode, equals(HttpStatus.ok));
          final body1 = await resp1.fold<List<int>>(
            [],
            (acc, c) => acc..addAll(c),
          );
          ...
          final body2 = await resp2.fold<List<int>>(
            [],
            (acc, c) => acc..addAll(c),
          );
    ...
    test(
      'returns 204 No Content within timeout when online upstream fails or is disconnected',
      () async {
        final fallbackServer = LocalTileServer(
          onlineFallbackEnabled: true,
        ...
        try {
          ...
          final req = await client.getUrl(
            Uri.parse('${fallbackServer.baseUrl}/tiles/9/9/9.pbf'),
          );
          final resp = await req.close();
          stopwatch.stop();
    ...
    test('serves terrain tile from online proxy and caches to disk', () async {
      ...
      final fallbackServer = LocalTileServer(
        ...
        onlineTerrainFallbackUrlTemplate:
            'http://127.0.0.1:${mockUpstream.port}/terrain/{z}/{x}/{y}.png',
        cacheDirectoryPath: tempDir.path,
      );
      ...
      try {
        ...
        // 1. Initial request triggers online proxy
        final req1 = await client.getUrl(
          Uri.parse('${fallbackServer.baseUrl}/terrain/8/135/88.png'),
        );
        final resp1 = await req1.close();
        expect(resp1.statusCode, equals(HttpStatus.ok));
        expect(
          resp1.headers.value(HttpHeaders.contentTypeHeader),
          equals('image/png'),
        );
        final body1 = await resp1.fold<List<int>>(
          [],
    ...
    test(
      'returns 204 No Content for missing terrain tile when offline',
      () async {
        final fallbackServer = LocalTileServer(
          onlineFallbackEnabled: true,
          onlineTerrainFallbackUrlTemplate:
              'http://127.0.0.1:1/terrain/{z}/{x}/{y}.png',
        );

        try {
          await fallbackServer.start();
          final req = await client.getUrl(
            Uri.parse('${fallbackServer.baseUrl}/terrain/10/1/1.png'),
          );
          final resp = await req.close();
          expect(resp.statusCode, equals(HttpStatus.noContent));
        } finally {
    ...
    test('does not include wildcard CORS headers on responses', () async {
      await server.start();
      await server.setFallbackArchive(overviewPath);

      final reqHealth = await client.getUrl(
        Uri.parse('${server.baseUrl}/health'),
      );
      final respHealth = await reqHealth.close();
      expect(
        respHealth.headers.value(HttpHeaders.accessControlAllowOriginHeader),
        isNull,
      );

      final reqTile = await client.getUrl(
        Uri.parse('${server.baseUrl}/tiles/0/0/0.pbf'),
      );
      final respTile = await reqTile.close();
      expect(
        respTile.headers.value(HttpHeaders.accessControlAllowOriginHeader),
        isNull,
      );
    });
  });
