...
Provides end-to-end automatic flight tracking with hybrid sensor/terrain fusion, local-first logbook management, multi-format export, real-time widget replay with a floating bottom HUD and variable speed control, and resilient offline-queued XContest submission.

## ADDED Requirements

### Requirement: Automatic takeoff detection with hybrid sensor fusion
The flight tracking system SHALL continuously evaluate GPS velocity, barometric climb rates, and terrain elevation (DEM height above ground) to automatically detect takeoff without manual pilot action, prepending a 15-second pre-takeoff circular buffer to ensure complete launch capture.
...
The replay controller SHALL display a persistent floating bottom HUD supporting play/pause, timeline scrubbing, timestamp display, exit action, and discrete speed multipliers (1x, 2x, 3x, 4x, 5x, 6x, 7x, 8x) that cycle in a continuous loop back to 1x upon successive taps.

#### Scenario: Cycling replay playback speed
- **WHEN** the user taps the speed button on the Replay HUD while at 1x speed
- **THEN** the speed increments to 2x, then 3x, sequentially up to 8x
- **AND WHEN** the user taps the speed button while at 8x speed
