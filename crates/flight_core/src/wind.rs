...
impl WindEstimator {
    ...
    pub fn update(&mut self, timestamp_ms: u64, heading_deg: f64, position: Position, is_circling: bool) {
        if !is_circling {
            self.reset();
            return;
        }
        ...
        if let Some(last) = self.last_heading {
            ...
            if self.current_turn_rotation.abs() >= 360.0 {
                let completion = TurnCompletion { timestamp_ms, position };
                ...
                if let Some(prev_completion) = self.turn_completions.last() {
                    ...
                    let dt_s = (completion.timestamp_ms - prev_completion.timestamp_ms) as f64 / 1000.0;
                    ...
                    if dt_s > 0.0 {
                        ...
                        let mut dir_deg = (f64::atan2(-vx, -vy) * 180.0 / std::f64::consts::PI + 360.0) % 360.0;
                        if dir_deg < 0.0 { dir_deg += 360.0; } // Should be handled by %, but be safe
                        ...
                        self.current_estimate = match self.current_estimate {
                            Some(old) => {
                                ...
                                Some(WindVector {
                                    speed_kmh: old.speed_kmh * (1.0 - self.alpha) + new_estimate.speed_kmh * self.alpha,
                                    // Properly averaging angles requires vector addition, but for now we'll average the components.
                                    velocity_x_ms: old.velocity_x_ms * (1.0 - self.alpha) + new_estimate.velocity_x_ms * self.alpha,
                                    velocity_y_ms: old.velocity_y_ms * (1.0 - self.alpha) + new_estimate.velocity_y_ms * self.alpha,
                                    direction_deg: {
                                        let vx_avg = old.velocity_x_ms * (1.0 - self.alpha) + new_estimate.velocity_x_ms * self.alpha;
                                        let vy_avg = old.velocity_y_ms * (1.0 - self.alpha) + new_estimate.velocity_y_ms * self.alpha;
                                        (f64::atan2(-vx_avg, -vy_avg) * 180.0 / std::f64::consts::PI + 360.0) % 360.0
                                    }
                                })
                            },
...
mod tests {
    ...
    fn test_wind_estimation() {
        ...
        assert!((wind.direction_deg - 270.0).abs() < 5.0, "Expected ~270, got {}", wind.direction_deg);
        assert!((wind.speed_kmh - 36.0).abs() < 1.0, "Expected ~36 kmh (10m/s), got {}", wind.speed_kmh);
    }
}
