...
impl SyntheticReplayGenerator {
    ...
    pub fn generate_fixture(&self, scenario: SyntheticReplayScenario) -> Vec<SensorEvent> {
        match scenario {
            ...
            SyntheticReplayScenario::ThermalingSpiral => self.generate_thermaling_spiral(600),
        }
    }
    ...
    fn generate_thermaling_spiral(&self, duration_s: usize) -> Vec<SensorEvent> {
        ...
        for seq in 1..=duration_s as u64 {
            ...
            events.push(SensorEvent {
                schema_version: NATIVE_PIPELINE_SCHEMA_VERSION,
                source_id: SensorSourceId::Gps,
                source_timestamp_ns: Some(current_time_ns),
                native_received_timestamp_ns: current_time_ns + 10_000,
                sequence: seq * 2 - 1,
                quality_flags: SensorQualityFlags::nominal(),
                payload: SensorPayload::Gps {
                    latitude_deg: lat,
                    longitude_deg: lon,
                    altitude_m: alt as f32,
                    ground_speed_mps: gnd_speed as f32,
                    bearing_deg: ground_bearing_deg as f32,
                    accuracy_m: 2.0,
                },
            });
            ...
            events.push(SensorEvent {
                schema_version: NATIVE_PIPELINE_SCHEMA_VERSION,
                source_id: SensorSourceId::Barometer,
                source_timestamp_ns: Some(current_time_ns + 10_000_000), // Offset slightly
                native_received_timestamp_ns: current_time_ns + 20_000,
                sequence: seq * 2,
                quality_flags: SensorQualityFlags::nominal(),
                payload: SensorPayload::Barometer {
            ...
            events.push(SensorEvent {
                ...
                source_timestamp_ns: Some(current_time_ns + 20_000_000),
                native_received_timestamp_ns: current_time_ns + 30_000,
                sequence: seq * 2 + 1,
                quality_flags: SensorQualityFlags::nominal(),
                payload: SensorPayload::Variometer {
                    climb_rate_mps: climb_rate as f32,
                    integrated_lift_mps: climb_rate as f32,
                },
            });
        }
